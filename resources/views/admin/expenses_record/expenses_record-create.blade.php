            ...
            <div class="col-sm-12">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            <label class="col-form-label pt-0" for="vendor">Vendor</label>
                            <select class="form-select" id="vendor" name="vendor" >
                                <option value="">Select</option>
                                @foreach ($vendor as $item)
                                <option @if(isset($post))  @if($post->vendor == $item->id) selected @endif @endif value="{{$item->id}}">{{$item->name}}</option>
                                @endforeach
                            </select>
                        </div>

                        <div class="mb-3">
                            <label class="col-form-label pt-0" for="account">Account</label>
                            <input class="form-control" id="account" value="{{ (isset($post)?$post->account:'')}}" name="account" type="number" max-length="10" aria-describedby="" placeholder="Enter account">
                        </div>

                        <div class="mb-3">
                            <label class="col-form-label pt-0" for="amount">Amount</label>
                            <input class="form-control" id="amount" value="{{ (isset($post)?$post->amount:'')}}" name="amount" type="number" aria-describedby="" placeholder="Enter amount">
                        </div>

                        <div class="mb-3">
                            <label class="col-form-label pt-0" for="approvers">Approvers</label>
                            <input class="form-control" id="approvers" value="{{ (isset($post)?$post->approvers:'')}}" name="approvers" type="text" aria-describedby="" placeholder="Enter approvers">
                        </div>
